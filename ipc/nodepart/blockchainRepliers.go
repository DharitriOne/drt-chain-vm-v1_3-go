...
func (part *NodePart) replyToBlockchainGetDCDTToken(request common.MessageHandler) common.MessageHandler {
	...
	result, err := part.blockchain.GetDCDTToken(typedRequest.Address, typedRequest.TokenID, typedRequest.Nonce)

	response := common.NewMessageBlockchainGetDCDTTokenResponse(result, err)
	return response
}
